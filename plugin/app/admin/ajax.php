...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    public function approve_solution_action()
    {
        $this->model('solution')->approve_solution_by_ids($_POST['solution_id'], $_POST['status']);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    public function solution_manage_action()
    {
        if (!$_POST['solution_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择需要操作的参考答案')));
        }

        switch($_POST['action'])
        {
            case 'remove' :
                $this->model('solution')->remove_solution_by_ids($_POST['solution_ids']);

                break;

            case 'show' :
                $this->model('solution')->approve_solution_by_ids($_POST['solution_ids'], 0);

                break;

            case 'hide':
                $this->model('solution')->approve_solution_by_ids($_POST['solution_ids'], 1);
