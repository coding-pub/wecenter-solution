...
class main extends AWS_CONTROLLER
{
	...
	public function question_action()
	{
		...
		$solution_info = $this->model('solution')->get_solution_info_by_question_id($question_info['question_id']);
		if($this->user_info['permission']['is_administortar'] || $this->user_info['permission']['is_moderator'] ||
			$this->user_id == $solution_info['published_uid'])
		{
		...
		{
			$user_solution = $this->model('solution')->get_user_solution_record($this->user_id, $solution_info['solution_id']);
		}
	...
	public function solution_action()
	{
		if ($_GET['id'])
		{
			if (!$question_info = $this->model('question')->get_question_info_by_id($_GET['id']))
			...
			$solution_info = $this->model('solution')->get_solution_info_by_question_id($_GET['id']);
		}
		...
		else if ($this->is_post() AND $_POST['solution_content'])
		{
			$solution_info = array(
				...
				'view_cost' => htmlspecialchars($_POST['view_cost'])
			);
		}
		...
		{
			$draft_content = $this->model('draft')->get_data(1, 'solution', $this->user_id);

			$solution_info = array(
				'solution_content' => htmlspecialchars($_POST['solution_content']),
				'view_cost' => htmlspecialchars($draft_content['view_cost'])
			);
		}
		...
		if (!$solution_info['view_cost'])
		{
			$solution_info['view_cost'] = ($_GET['view_cost']) ? intval($_GET['view_cost']) : 50;
		}
	...
	{	
		...
		if ($_GET['id'])
		{
			...
			else
			{
				if($_GET['ask'])
				{
					$view_cost = $solution_info['view_cost'];
					$question_id = $solution_info['question_id'];
					TPL::assign('view_cost', $view_cost);
					TPL::assign('question_id', $question_id);
					TPL::output('m/solution_view_ask');
					$direct_view = False;
				}
				else
				{
